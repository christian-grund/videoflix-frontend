...
export class VideoplayerComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit() {
    if (this.videoPlayer) {
      ...
      videoPlayer.addEventListener('timeupdate', () => {
        ...
        this.updateProgress(); // Update progress here
  ...
  onMetadataLoaded(event: Event) {
    ...
    this.videoDuration = video.duration; // Setze die Videodauer
    console.log('Metadata loaded, video duration:', this.videoDuration);
  }

  onTimeUpdate(event: Event) {
    const video: HTMLVideoElement = this.videoPlayer.nativeElement;
    this.currentTime = video.currentTime; // Aktualisiere die aktuelle Zeit
    this.updateProgress(); // Aktualisiere die Fortschrittsanzeige
  ...
  seek(event: MouseEvent) {
    ...
    if (video.readyState > 2 && video.duration) {
      ...
      console.log('Setting new currentTime:', newTime);
      video.currentTime = newTime; // Setze die neue Zeit
    } else {
